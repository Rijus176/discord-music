...
module.exports = async (client) => {
  let docs = `# Command List - ${pkg.name}  \n> Command list generated [here](https://github.com/NekoYasuii/${pkg.name}/blob/main/src/scripts/docs.js)\n`;
...
${client.commands.filter((c) => c.category === category).map((command) =>
  `| [${command.id}](https://github.com/NekoYasuii/${pkg.name}/blob/main/docs/commands.md#${command.id}) | ${command.description || "No description provided."} |`
).join("\n")}`;
...
${client.commands.filter((c) => c.category === category).map((command) =>
  ...
  docs += `\n\n\n[Back to top](https://github.com/NekoYasuii/${pkg.name}/blob/main/docs/commands.md#command-list---${pkg.name})`;
